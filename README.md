...
| 운영 | 콘텐츠 단일 설정 파일 관리, 모바일 복귀/저전력 대응, 끊김 사유 안내 등 **운영 친화 설계** |
...
- **코드 작성/수정**: 웹↔Unity 브리지(`SendMessage`/`.jslib`), 방명록 모듈, 로딩/복귀 처리 등 반복 패턴 코드를 AI와 함께 빠르게 구현·리팩터링.
- **엔진 GUI 작업의 자동화**: 본래 Unity 에디터에서 마우스로 하던 작업(씬 열기, 컴포넌트 값 설정, 빌드 설정 변경, 화면 검증)을 **MCP를 통해 AI가 직접 수행** → 반복 클릭 제거.
...
**경로 B — 3D 예식장(Unity 빌드): 단일 명령 멱등 배포 (`deploy-venue.ps1`)**
1. Unity 빌드 산출물(`Build/`, `TemplateData/`) → 배포 폴더로 복사
2. 웹 래퍼(`venue.html` → `index.html`) + 에셋(js/css/favicon) 복사
3. `rclone copy → r2:jshw-venue` — **체크섬 비교로 변경된 파일만 업로드**
...
- **반응형 화면**: 모바일 우선 레이아웃, `dvh`/safe-area 대응(모달 상단 잘림 수정), 모바일 키보드 입력 이슈(WebGLInput) 처리.
- **데이터 관리**: 콘텐츠/사진/계좌/좌표를 **`config.js` 한 곳(SSOT)** 에서 관리. 방명록은 Firestore에 `{name, message, createdAt}`로 저장, 페이징·실시간 반영.
- **배포 관련 기능**: 멱등 배포 스크립트(`deploy-venue.ps1`), rclone 증분 업로드, R2 단일 오리진 서빙, 배포 후 HEAD 검증.
- **운영 친화 기능**: 이름 기억 → **자동 재입장**(iOS 백그라운드 복귀 대응), 접속 지연 시 안내+새로고침, **끊김 사유 화면 노출**, 관리자 인증 삭제 도구.
...
**정직한 한계 / 다음 단계**: 배포 *절차*는 자동·멱등이지만 *시작 트리거*는 수동(빌드 후 실행)입니다. `venue.html`·배포 스크립트가 로컬 전용이라 디스크가 유일 원본인 점도 개선 여지입니다 → 다음 단계로 **빌드 훅 트리거 연결, 스크립트 버전관리, 커스텀 도메인(r2.dev rate-limit 제거)** 을 계획했습니다.
